...
class HomeView(ListView):
    ...
    extra_context = {
        "title": "De Voordeligste Huisdierenvoeding Online | Dierenvoedingexpert"
    }
    ...
    def get_context_data(self, **kwargs):
        ...
        context["uitgelicht"] = Product.objects.all().order_by("-sale_price")[:12]
        context["products"] = Product.objects.all()
...
class AanbiedingView(ListView):
    ...
    extra_context = {
        "title": "De Beste Deals Voor Al Jouw Dierenvoer | Dierenvoedingexpert"
    }
    paginate_by = 36

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        all_products = Product.objects.filter(sale__gt=0)
        queryset = self.get_queryset().filter(sale__gt=0)
        ...
        context["animals"] = all_products.values_list("animal", flat=True).distinct()
        context["brands"] = (
            all_products.values_list("brand", flat=True).distinct().order_by("brand")
        )
...
class AnimalListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        all_animal_products = self.model.objects.filter(animal=self.animal).exclude(
            category="Apotheek"
        )
        queryset = (
            self.get_queryset().filter(animal=self.animal).exclude(category="Apotheek")
        )
        print(queryset[:5])
        context["count"] = queryset.count()
        context["max_pages"] = math.ceil(queryset.count() / self.paginate_by)
        ...
        context["brands"] = (
            all_animal_products.values_list("brand", flat=True)
            .distinct()
            .order_by("brand")
        )
        return context
...
class HondListView(AnimalListView):
    ...
    extra_context = {"title": "Hondenbrokken, Natvoer en Snacks | Dierenvoedingexpert"}
...
class KatListView(AnimalListView):
    ...
    extra_context = {
        "title": "Alle Voeding En Snacks Voor Jouw Kat | Dierenvoedingexpert"
    }
...
class KnaagdierListView(AnimalListView):
    ...
    extra_context = {
        "title": "Voer Voor Konijnen, Cavia's, Hamsters En Ratten | Dierenvoedingexpert"
    }
    categorys = []
...
class ApotheekListView(ListView):
    ...
    extra_context = {
        "title": "Alle Medicijnen En Hulpmiddelen Voor Jouw Dier | Dierenvoedingexpert"
    }
    paginate_by = 36

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        all_medicine_products = Product.objects.filter(category="Apotheek")
        queryset = self.get_queryset().filter(category="Apotheek")
        context["count"] = queryset.count()
        context["max_pages"] = math.ceil(queryset.count() / self.paginate_by)
        paginator = Paginator(queryset, self.paginate_by)
        page = self.request.GET.get("page")
        try:
            ...
            product_list = paginator.page(paginator.num_pages)
        context["products"] = product_list
        context["animals"] = all_medicine_products.values_list(
            "animal", flat=True
        ).distinct()
        ...
        context["brands"] = (
            all_medicine_products.values_list("brand", flat=True)
            .distinct()
            .order_by("brand")
        )
        animal = self.request.GET.get("animal")
...
class OverOnsView(TemplateView):
    ...
    extra_context = {"title": "Over Ons | Dierenvoedingexpert"}
...
class ZoekView(ListView):
    ...
    extra_context = {"title": "Zoeken Naar Jouw Product | Dierenvoedingexpert"}
    paginate_by = 36
    ...
    def get_context_data(self, **kwargs):
        ...
        context["max_pages"] = math.ceil(all_search_products.count() / self.paginate_by)
